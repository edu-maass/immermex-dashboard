"""
Script de migración que maneja la estructura real de las tablas
Verifica la estructura de compras_v2 y ajusta la migración
"""
...
def check_table_structures():
    ...
    try:
        ...
        # Verificar estructura de compras_v2
        cursor.execute("""
            SELECT column_name, data_type, is_nullable
            FROM information_schema.columns 
            WHERE table_name = 'compras_v2' AND table_schema = 'public'
        ...
        # Verificar estructura de facturacion
        cursor.execute("""
            SELECT column_name, data_type, is_nullable
            FROM information_schema.columns 
            WHERE table_name = 'facturacion' AND table_schema = 'public'
            ORDER BY ordinal_position
        """)
        facturacion_columns = cursor.fetchall()
        
        logger.info("Estructura de tabla 'facturacion':")
        for col in facturacion_columns:
            logger.info(f"  - {col['column_name']}: {col['data_type']}")
        
        cursor.close()
        conn.close()
        
        return compras_v2_columns, facturacion_columns
        
    except Exception as e:
        logger.error(f"Error verificando estructuras: {str(e)}")
        conn.close()
        return False, False

def migrate_without_foreign_keys():
    """Migra datos sin usar claves foráneas problemáticas"""
    conn = get_supabase_connection()
    if not conn:
        return False
    ...
    try:
        ...
        for pedido in pedidos_data:
            try:
                ...
                # Insertar en pedidos_compras sin compra_imi (usar NULL)
                cursor.execute("""
...
def main():
    """Función principal"""
    print("MIGRACION SIMPLIFICADA PARA SUPABASE")
    print(f"Hora: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*60)
    
    try:
        # Verificar estructuras de tablas
        logger.info("Verificando estructuras de tablas...")
        compras_columns, facturacion_columns = check_table_structures()
        
        # Ejecutar migración simplificada
        logger.info("Iniciando migración simplificada...")
        migration_success = migrate_without_foreign_keys()
