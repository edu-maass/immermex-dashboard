"""
Script para migrar compras manteniendo los IMI originales
Usa los valores de IMI de la tabla compras original en lugar de generar nuevos
"""
...
def group_compras_by_factura_with_imi():
    """Agrupa las compras por número de factura manteniendo el IMI original"""
    conn = get_supabase_connection()
    if not conn:
        return None
...
def migrate_compras_with_original_imi():
    """Migra datos de compras a compras_v2 y compras_v2_materiales usando IMI original"""
    conn = get_supabase_connection()
    if not conn:
        return False
    ...
    try:
        ...
        # Obtener compras agrupadas por factura con IMI original
        grouped_compras = group_compras_by_factura_with_imi()
        
        if not grouped_compras:
            logger.info("No hay compras para migrar")
            return True
        
        logger.info(f"Iniciando migración con IMI originales")
...
def main():
    """Función principal"""
    print("MIGRACION CON IMI ORIGINALES")
    ...
    try:
        ...
        # Ejecutar migración con IMI originales
        logger.info("Iniciando migración con IMI originales...")
        migration_success = migrate_compras_with_original_imi()
        ...
        if migration_success:
            ...
            print("\n" + "="*60)
            print("RESUMEN DE MIGRACION CON IMI ORIGINALES")
            print("="*60)
            ...
            if verification['migration_successful']:
                logger.info("MIGRACION CON IMI ORIGINALES EXITOSA!")
