"""
Script para limpiar y re-ejecutar la migración completa
Elimina todos los registros de compras_v2 y compras_v2_materiales y ejecuta la migración de nuevo
"""
...
def verify_final_migration():
    """Verifica que la migración final se completó correctamente"""
    conn = get_supabase_connection()
    if not conn:
        return False
    ...
    try:
        ...
        logger.info(f"Verificación final de migración:")
...
def main():
    """Función principal"""
    print("LIMPIEZA Y MIGRACION COMPLETA DE COMPRAS")
    ...
    try:
        ...
        if migration_success:
            # Verificar migración
            verification = verify_final_migration()
            
            print("\n" + "="*60)
            print("RESUMEN DE MIGRACION COMPLETA")
            print("="*60)
            ...
            if verification['migration_successful']:
                logger.info("MIGRACION COMPLETA EXITOSA!")
                logger.info("Las tablas han sido limpiadas y migradas correctamente")
                return True
            else:
